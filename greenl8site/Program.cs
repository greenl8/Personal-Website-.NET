            ...
            using (var scope = host.Services.CreateScope())
            {
                ...
                try
                {
                    ...
                    if (canConnect)
                    {
                        ...
                        // Ensure primary key IDs auto-generate in PostgreSQL for tables migrated from SQLite
                        await EnsureTableIdAutoGenerationAsync(context, logger, "Posts");
                        await EnsureTableIdAutoGenerationAsync(context, logger, "Pages");
                        await EnsureTableIdAutoGenerationAsync(context, logger, "Categories");
                        await EnsureTableIdAutoGenerationAsync(context, logger, "Tags");
                    }
                    else
                    {
...
END $$;";
        ...
        private static async Task EnsureTableIdAutoGenerationAsync(AppDbContext context, ILogger logger, string tableName)
        {
            try
            {
...
BEGIN
  IF EXISTS (
    ...
    -- If Id is not identity, try to convert it
    IF NOT EXISTS (
      SELECT 1
      FROM pg_attribute a
      JOIN pg_class t ON a.attrelid = t.oid
      ...
      BEGIN
        -- Drop any default first to avoid conflicts
        EXECUTE 'ALTER TABLE ""{tableName}"" ALTER COLUMN ""Id"" DROP DEFAULT';
        -- Try to add identity
    ...
    -- If still not identity, ensure sequence default and align it
    IF NOT EXISTS (
      SELECT 1
      FROM pg_attribute a
      JOIN pg_class t ON a.attrelid = t.oid
      ...
      IF NOT EXISTS (
        ...
        WHERE s.relkind = 'S'
          AND n.nspname = current_schema()
          AND s.relname = '{tableName}_Id_seq'
      ) THEN
        EXECUTE 'CREATE SEQUENCE ""{tableName}_Id_seq""';
      END IF;
      ...
      IF NOT EXISTS (
        ...
        EXECUTE 'ALTER TABLE ""{tableName}"" ALTER COLUMN ""Id"" SET DEFAULT nextval(''""{tableName}_Id_seq""''::regclass)';
        EXECUTE 'ALTER SEQUENCE ""{tableName}_Id_seq"" OWNED BY ""{tableName}"".""Id""';
      END IF;

      -- Align sequence with current max(Id)
      EXECUTE 'SELECT setval(''""{tableName}_Id_seq""''::regclass, COALESCE((SELECT MAX(""Id"") FROM ""{tableName}""), 0))';
    END IF;
  END IF;
END $$;";

                await context.Database.ExecuteSqlRawAsync(sql);
                logger.LogInformation($"Ensured {tableName}.Id auto-generation and sequence alignment");
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, $"Failed to enforce {tableName}.Id auto-generation. Continuing.");
            }
        }
